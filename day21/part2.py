...
def shortest_seq(ch1, ch2, pads):
    ...
    costs = { }
    states = []
    heapq.heappush(states, (0, 0, px, py, "A"))
    out_seq = None
    while states:
        ...
        if (x, y, p_ch) not in costs:
            ...
            assert costs[(x, y, p_ch)] <= c
            continue

        if (x, y) == (tx, ty):
            found_seq = s + shortest_seq(p_ch, "A", next_pads)
            if out_seq is None or found_seq < out_seq:
                out_seq = found_seq
        ...
        for (x2, y2, ch) in [(x - 1, y, "<"), (x + 1, y, ">"), (x, y - 1, "^"), (x, y + 1, "v")]:
            ...
            heapq.heappush(states, (new_seq, new_seq, x2, y2, ch))
...
for seq in open(0):
    seq = seq.strip()
    total += int(seq[:-1]) * final_seq(seq, (keypad,) + (dirpad, )*25)
print(total)
